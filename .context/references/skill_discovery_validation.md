...
- **DIAYN skill collapse** (URLB benchmark Laskin 2021, then partially rebutted by CIC Laskin 2022): URLB found "no competence-based approach achieves SOTA on URLB" at the time of writing — DIAYN learns small static skill sets, sensitive to alpha (entropy), num_skills, and early-termination dynamics. CIC subsequently broke this barrier (CIC IS a competence-based approach achieving SOTA — re-attribution from prior version of this doc). On DM Control Hopper without early reset, "both algorithms collapse." Specific Walker-Run 2M+100k benchmark numbers vary by source; CIC Table 6 reports DIAYN ≈158 ± 8 vs APT ≈392 ± 26 (verify exact values against the CIC paper before citing).
...
| **DADS** | continuous in `[-1,1]^D` | `I(s'; z \| s)` via forward dynamics model `q_φ(s'\|s,z)`; planning via MPPI in skill space | predictable transitions, **zero-shot composition without RL**, off-DADS deployed real-robot | improper bound (Appendix C), x-y prior often required, off-DADS 20 robot-hours per real run, sample-inefficient | **Yes via off-DADS follow-up** (D'Kitty) — original DADS arXiv 1907.01657 is sim-only; off-DADS arXiv 2004.12974 is the hardware paper |
...
6. ~~Patch SD-A plan acceptance with the s-not-s' fix and M=10 per-skill rollout convention.~~ **Done** — SD-A plan Task 4/5 carries the DIAYN-on-s warning; M=10 sim / M=3 hardware fixed in spec SD-C/E acceptance gates.
