...
### Checkpoint format
Every checkpoint contains: `meta.json` (full config), `metrics.csv` (training curve), `actor_params.npy` (inference), `orbax/` (training resume). `load_actor_for_inference()` loads just actor_params.npy — no orbax needed. **`meta["control"]["osc"]` (added 2026-06-13) self-describes the OSC/var-impedance controller** (use_op_space_inertia, granularity, damping/mass_action, var_{s,zeta,a} ranges, var_xdd_ema, kp/kd) so the Newton eval harness (`mud_osc`) configures from the ckpt instead of guessing — never infer controller identity from action_dim (it's ambiguous). A jax↔numpy parity test (`test_jax_numpy_osc_parity`) pins the two OSC ports equal.

### Available Go2 envs

**55 Go2 Warp variants are declared in `jax_rl/envs/locomotion/go2_warp_variants.py`** — that table is the single source of truth (one `EnvVariant` per env: config knobs + host class + train overrides + research notes). Families: joint-PD (Flat/TorqueSpeed/NoAccel/Unitree), fixed-gain OSC (+Jᵀ ablation, Kp sweep), variable impedance (per-foot/per-axis, ±damping, ±virtual-mass), flat-physical motor, hard-kick ladder, rough heightfield, curriculum, PosTrack proto, **mud-DR + reward-profile family** (slow+firm/NoAir/RMA/gait_participation; the Newton-mud research line). **The 21 OSC / physical-motor / rough variants default to `reset_mode=per_step` (DR ON) + `eval_every_n_episodes=500` since 2026-06-11** — OSC runs from before that date trained without DR (silent preset fallback) and are not comparable. Highlights:
...
| `Go2WarpSplitbelt` | Two belts butted at y=0 (left half-width 0.150 at body pos -0.150, right at +0.150), over `fallback_floor` outside `|y|>0.300` | Ideal PD | Adaptation benchmark substrate (A1/A2/A3/A4). `tied(0.5)` default. Obs schema aligned to `Go2WarpJoystickFlatNoAccel` — joystick ckpts pop in directly. v2 eval 72.3 ± 13.4 INVALIDATED by tunneling fix 2026-05-06 (untrained on new env). |
| `Go2WarpSplitbeltDR` | Same apparatus | Ideal PD | Belt-speed DR variant. `random_per_episode` (v∈[0.3,1.5], ratio∈[0.5,2.0]). Same obs as Splitbelt. A2-prep. Untrained on fixed env. |
| `Go2WarpSplitbeltPoseDR` | Same apparatus | Ideal PD | Pose-track: actor sees world body pos + upvec + fwdvec (idealized, NOT deployable). Reward = pos+orient tracking. `random_per_episode`. **v2 eval 378.9 ± 113.4** at 1M (`checkpoints/20260506_195126_fast_sac_go2warpsplitbeltposedr_seed0`, retrained 2026-05-06 on tunneling-fixed env). v1 280.6 INVALIDATED (foot-as-anchor exploit). OOD sweep: peak v=0.5 (393), graceful degradation at v=2.5 (30, 8% of peak). |
