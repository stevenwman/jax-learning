...
- **`record_video.py` needs `XLA_PYTHON_CLIENT_PREALLOCATE=false` on contested GPU** — JAX preallocator grabs a fixed slice upfront, leaving no room for Warp's CUDA graph capture (~1-2 GiB transient). On-demand allocation lets them share. The 1.12 MiB OOM after 13.6 GiB free is the giveaway.
