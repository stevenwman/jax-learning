...
- **Seed 1 of Classic — return rank ≠ xy spread** — z6=+199.6 (highest return) is a stand-upright/shuffle skill near origin. z7=-227 (negative return) is the 1.4m -x trail that drives max-pairwise. Lesson: in Ant, "best return" and "biggest displacement" can be opposite skills because survive_reward dominates if the policy stays healthy.
...
- **Every var-impedance/OSC policy parks the back-right foot (RR-hang)** — contact raster (replay qpos → mj_forward → floor sensors) shows RR ~0% duty across 4 rewards + the new mass controller; joint-PD is symmetric. Joint-cycle count is MISLEADING (RR joints cycle while the foot stays airborne — measure CONTACT, not joint motion). It's a learned 3-legged optimum var-impedance can reach, NOT a hard geometry lock.
- **`gait_participation` (anti-leg-park) reward fixes the RR-hang** — penalize steps-since-all-four-feet-last-touched (scale −2.0). RR contact 0%→32%, all four feet plant, eval unchanged (~276 vs 283). The leg-park is reward-fixable after all; earlier reward attempts (RMA-minimal, NoAir) didn't because they lacked an explicit participation penalty.
- **Finite-diff across a step can silently difference a value with itself (ẍ≡0)** — the virtual-mass ẍ=(v_now−v_prev)/dt was EXACTLY 0 for all training: `v_prev` stored at end-of-step (post-control), `v_now` read at start-of-next-step = the SAME carried `state.data` → diff≡0, mass term DEAD (secretly bare-K/D). Hid because tracking eval (283) ignores a dead aux term, and the Newton eval harness computed ẍ correctly → term went live only at transfer (looked like instability). Fix: update `v_prev` at START of step (same phase as `v_now`). Rule: instrument zeroable terms directly; a finite-diff needs two DIFFERENT-time samples. See `lessons/go2.md`.
- **Acceleration-feedback virtual mass (F=A·ẍ+K·err+D·ẋ) is stable on flat, CATAPULTS on Newton mud** — policy commands per-axis virtual mass A, ẍ=finite-diff foot vel. Flat MJX trains fine (eval 283); on MPM mud the sharp contact transients spike ẍ and A·ẍ (a_max=2.0) flings the robot 2 m up + flips. Clamping a_max→0.5 stops it. The mass *range* is the stability lever; the spec's a_max=2.0 guess was too large for the rougher substrate. See `2026-06-12-osc-virtual-mass-design.md`.
...
- **Don't let the eval harness GUESS controller params** — porting the OSC/mass controller to mud_osc bit 3× (each a "catapult" that looked like instability but was a Λ-vs-bare / a_max / dropped-mass MISMATCH). Root cause = harness inferring params meta didn't store, NOT bad math. Fix: ckpt SELF-DESCRIBES its controller (`meta['control']['osc']`) + a jax↔numpy parity test pins the two ports equal. Inferring controller identity from `action_dim` is a trap (48-d = ambiguous bare-mass vs Λ-mass). See `lessons/newton_mud_eval.md`.
