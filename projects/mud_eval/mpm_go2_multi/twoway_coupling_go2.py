...
def compute_body_forces(
    ...
    if cid >= 0 and cid < body_ids.shape[0]:
        ...
        f_world = collider_impulses[i] / dt # force magnitude and direction in world coordinates

        X_wb = body_q[body_index] # body pose in world coordinates 
        X_com = body_com[body_index] # center of mass position in local coordinates
        r = collider_impulse_pos[i] - wp.transform_point(X_wb, X_com) # point in world where there is an impulse - the transformed body com position, gives us the lever arm to compute torque
        wp.atomic_add(body_f, body_index, wp.spatial_vector(f_world, wp.cross(r, f_world))) # directly adds forces onto the body state
