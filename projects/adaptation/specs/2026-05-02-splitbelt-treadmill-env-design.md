...
- **Belt initial qvel + sign convention:** Schedule semantics follow biomech — `schedule_table[t] > 0` means "belt drags foot **backward** at this speed." Belt slide joint axis is +x in the apparatus XML, robot faces +x, so the actual joint velocity written to MJX is **negated**: `data.ctrl[belt_act] = -schedule_table[t]` and `qvel[belt_dofadr] = -schedule_table[0]` at reset. The slide-joint qvel is negative when schedule is positive (slab moves -x → drags foot -x → robot must step +x to stay over treadmill center). Belt actuator gain (`kv=200`) settles within one ctrl_dt either way; we accept the first-step transient.
- **`step_idx` key location:** `info["step_idx"]` is the canonical top-level field used for schedule indexing and incrementing. `info["splitbelt"]["step_idx"]` is a logged copy for offline analysis (so the .npz traj knows which schedule row each frame corresponds to). Both are written from the same source; do not let them diverge.
...
| Policy locks joints, gets dragged | Drift penalty + episode timeout truncation; smoothness terms (`pose`, `feet_air_time`, `feet_clearance`, `feet_height`) collectively shape stepping. Joystick `survival` term was dropped along with `stand_still` — neither exists in the splitbelt reward set. |
...
- **Reward zero-clip + PPO entropy collapse.** Per joystick precedent, the env clips total reward via `jp.clip(sum(rewards) * dt, 0.0, 10000.0)`. Bad early policies (large drift + termination penalty) hit a flat-zero reward floor; PPO's policy-gradient signal goes to zero on those steps and entropy can collapse before any positive signal appears. Mitigation if observed in Task 5.1 PPO smoke: lift the lower clip to `−inf` or a small negative bound (e.g. `−10`). Do not retune downstream weights instead — the clip is the actual cause.
