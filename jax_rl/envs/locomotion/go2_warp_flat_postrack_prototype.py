    ...
    return config_dict.create(
        ...
        reward_config=config_dict.create(
            scales=config_dict.create(
                ...
                pose=0.5,
                base_height=-5.0,
                # Unified position-tracking against marching target_xy. Off
                # by default; PosTrack preset enables. Lorentzian kernel
                # over (body_xy - target_xy) — heavy tails, no exp collapse.
        ...
        # If True, append `pos_delta_to_target_xy` (R^2) to actor `state`.
        # Required for pos_track_unified reward to be observable. Off by
        # default to preserve obs schema for existing joystick ckpts.
        pos_track_obs=False,
        impl="warp",
...
class WarpJoystick(go2_warp_base.Go2WarpEnv):
    ...
    def _post_init(self) -> None:
        ...
        self._obs_groups = {
            ...
            ] + ([ObsTerm("pos_delta_to_target_xy",
                          lambda data, info, **kw: data.qpos[:2] - info["target_xy"])]
                 if self._config.pos_track_obs else []),
            "privileged_state": [
                IncludeGroup("state"),
        ...
        self._reward_spec = [
            ...
            RewardTerm("pos_track_unified", lambda data, info, **kw:
                self._reward_pos_track_unified(data.qpos[:2], info["target_xy"])),
        ]
    ...
    def reset(self, rng: jax.Array) -> mjx_env.State:
        ...
        # Unified position-tracking target. Init at body_xy so first reward
        # is at zero lag. Advances at cmd*dt per step (see step()).
        target_xy_init = data.qpos[:2]
    ...
    def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
        ...
        # Advance target_xy by one step's commanded displacement. cmd=[0,0,0]
        # → fixed (matches splitbelt PosTrack semantic).
        state.info["target_xy"] = state.info["target_xy"] + state.info["command"][:2] * self.dt
    ...
    def _reward_pos_track_unified(
        self, base_xy: jax.Array, target_xy: jax.Array
    ) -> jax.Array:
        """Lorentzian position tracking. Matches splitbelt env's term so the
        same parameterization runs in both envs (target marches at cmd*dt;
        cmd=0 → target stationary)."""
        dx = base_xy[0] - target_xy[0]
        dy = base_xy[1] - target_xy[1]
