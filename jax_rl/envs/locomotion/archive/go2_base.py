    ...
    def __init__(
        ...
        for i in range(self._mj_model.nu):
            ...
            if 'calf' in name.lower():
                self._mj_model.actuator_ctrlrange[i] = np.array([-45.43, 45.43])
                self._mj_model.actuator_forcerange[i] = np.array([-45.43, 45.43])
            else:
                self._mj_model.actuator_ctrlrange[i] = np.array([-23.7, 23.7])
                self._mj_model.actuator_forcerange[i] = np.array([-23.7, 23.7])
        ...
        for jname in ["RL_thigh_joint", "RR_thigh_joint"]:
            ...
            self._mj_model.jnt_range[jid] = np.array([-0.5236, 4.5379])
