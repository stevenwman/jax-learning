...
class TerrainCurriculumDRWrapper(DomainRandWrapper):
    ...
    def reset(self, rng: jax.Array) -> mjx_env.State:
        ...
        state.info["is_goal_directed"] = jp.ones(self._num_envs, dtype=jp.bool_)
    ...
    def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
        ...
        # Unified advancement (all types goal-directed, rim-to-center):
        # reached → promote; fall OR no-progress-to-goal → demote
        reached = prev_reached
        no_progress = (~reached) & (~fall_at_done) & (prev_min_dist > 0.5 * prev_initial_dist)
        promote = reached & (~fall_at_done) & (done > 0)
        demote = (fall_at_done | no_progress) & (done > 0)
