...
Implementation:
  ...
  - JIT-compiled minibatch step and action selection via closures
  - Python loops for epochs/minibatches (unrolled at trace time if
    the outer update is ever jitted)
"""
...
class PPO:
    ...
    def update(self, state: TrainingState, batch: RolloutBatch, key: jax.Array) -> tuple[TrainingState, dict]:
        ...
        for epoch in range(self.config.num_epochs):
            ...
            for start in range(0, obs.shape[0], self.config.minibatch_size):
                ...
                # Dispatch to JIT-compiled minibatch step
                state, metrics = self._minibatch_step(
                    state, obs[mb], actions[mb], old_log_probs[mb], returns[mb], advantages[mb]
                )

                # Accumulate metrics for averaging across all minibatch updates
                if not metrics_accum:
                    metrics_accum = {k: v for k, v in metrics.items()}
                else:
                    for k, v in metrics.items():
                        metrics_accum[k] = metrics_accum[k] + v
    ...
    def select_action(
        ...
        Args:
            state: Training state with actor/critic params
