...
def train(cfg: TrainConfig, seed: int = 0):
    # ── Environment ──────────────────────────────────────────────────────
    ...
    samples_per_iter = cfg.num_envs * cfg.num_steps
    ...
    print(f"  num_envs={cfg.num_envs}, num_steps={cfg.num_steps}, episode_length={cfg.episode_length}")
    print(f"  samples/iter={samples_per_iter:,}, iterations={num_iterations}, total_steps={cfg.total_timesteps:,}")

    # ── PPO setup ────────────────────────────────────────────────────────
    ppo_cfg = cfg.ppo
    minibatch_size = min(2048, samples_per_iter)
    num_minibatches = samples_per_iter // minibatch_size
    total_gradient_steps = num_iterations * ppo_cfg.num_epochs * num_minibatches

    # Populate network/env-derived fields into the algo config
    ppo_config = dataclasses.replace(
        ppo_cfg,
        encoder=EncoderConfig(obs_dim=obs_dim, hidden_dim=cfg.hidden_dim),
    ...
    print(f"  hidden_dim={cfg.hidden_dim}, minibatch_size={minibatch_size}, "
          f"num_epochs={ppo_cfg.num_epochs}, grad_updates/iter={num_minibatches * ppo_cfg.num_epochs}")
    ...
    meta = {"env_name": cfg.env_name, "hidden_dim": list(cfg.hidden_dim),
...
if __name__ == "__main__":
    ...
    if args.hidden_dim is not None:
        overrides["hidden_dim"] = tuple(args.hidden_dim)
