...
def _rollout_once(algo, actor_params, reward_mode, obs_type, frame_stack, action_repeat, seed, max_steps, deterministic, key):
    env = make_eval_env(reward_mode, obs_type, frame_stack, action_repeat)
...
def evaluate(algo, actor_params, reward_mode: str, obs_type: str = "state",
             frame_stack: int = 1, action_repeat: int = 1,
             n_episodes: int = 5, max_steps: int = 300) -> dict:
    ...
    for i in range(n_episodes):
        r, c, s = _rollout_once(algo, actor_params, reward_mode, obs_type, frame_stack, action_repeat,
                                 1000 + i, max_policy_steps, True, key)
        det_ret.append(r); det_cov.append(c); det_succ.append(s)
        key, sk = jax.random.split(key)
        r, c, s = _rollout_once(algo, actor_params, reward_mode, obs_type, frame_stack, action_repeat,
                                 1000 + i, max_policy_steps, False, sk)
...
def train(
    ...
    # Env
    env = make_vec_env(num_envs, reward_mode, obs_type, frame_stack, action_repeat)
    ...
    for step in range(0, total_timesteps, num_envs):
        ...
        if (step + num_envs) % eval_every_n_steps < num_envs and buffer.size >= min_buffer_size:
            eval_stats = evaluate(algo, training_state.actor_params, reward_mode, obs_type, frame_stack, action_repeat)
